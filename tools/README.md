...
```bash
npm ci --ignore-scripts           # once: installs the pinned terser that minify.mjs runs
...
node tools/patch_shells.mjs       # shells: meta, canonical, JSON-LD, prerender, import map, fonts, cache-busting hashes
...
node tools/cf_purge.mjs           # drops the CDN copies of everything under assets/ (run after every deploy)
```

## Third-party code and fonts

Pages load nothing from other origins. `assets/vendor/three@0.160.0/` holds only the files the hero
and `studio.html` import, copied with the package layout (and its LICENSE) from `npm pack three@0.160.0`,
whose tarball npm checks against the registry integrity hash. The import map pointing at it is written
by `patch_shells.mjs`, into every shell and into `studio.html`, so a version bump means a new vendor
folder plus the `THREE_BASE` constant there.

`assets/fonts/` is generated by `node tools/build_fonts.mjs`: it fetches the Google Fonts CSS the site
used to link, downloads every woff2 it lists and writes `fonts.css` with the same `@font-face` rules.
File names carry the upstream font version, so they can stay cached for a year. Re-run it only to take
a new font version, then run `patch_shells.mjs`.
...
(it expects `python3 tools/serve.py 8899`).
...
```bash
python3 tools/serve.py            # http://localhost:8765, pass another port as the first argument
```

It behaves like GitHub Pages (extension-less paths like `/about`, directory indexes, `404.html`),
listens on 127.0.0.1 only and refuses dot-files and anything outside the repo.
